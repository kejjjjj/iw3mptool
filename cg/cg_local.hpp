...
struct radiantFace_t
{
	...
	float* windingPts[16];	// max winding-points per face = 16
};
...
struct pml_t
{
	...
	//{
	//	pml_t pml;
	//	memcpy_s(&pml, sizeof(pml_t), this, sizeof(pml_t));
	//	return pml;
	//}
};
struct pmove_t
{
	...
	//{
	//	pmove_t pm;
	//	memcpy_s(&pm, sizeof(pmove_t), this, sizeof(pmove_t));
	//	return pm;
	//}
};
...
public:
	...
	float* lerpOrigin; //0x001C
	float* lerpAngles; //0x0028
	char pad_0x0034[0x40]; //0x0034
	float* newOrigin; //0x0074
	char pad_0x0080[0x4C]; //0x0080
	int clientNum; //0x00CC
	unsigned int eType; //0x00D0
	unsigned int eFlags; //0x00D4
	char pad_0x00D8[0xC]; //0x00D8
	float* oldOrigin; //0x00E4
...
struct Material
{
	...
	const char* colormap() const
	{
		for (int i = 0; i < textureCount; i++)
			if (textureTable[i].is_color())
				return textureTable[i].u.image->name;

		return 0;

	}
	const char* normalmap() const
	{
		for (int i = 0; i < textureCount; i++)
			if (textureTable[i].is_normal())
				return textureTable[i].u.image->name;

		return 0;

	}
	const char* specularmap() const
	{
		for (int i = 0; i < textureCount; i++)
			if (textureTable[i].is_specular())
				return textureTable[i].u.image->name;
...
struct gentity_s
{
	...
	int count;
	gentity_s* chain;
	$4C7580D783CB81EAAF8D9BB4061D1D71 item;
...
struct refdef_s
{
	...
	int centerX() const noexcept
	{
		return x + width / 2;
	}
	int centerY() const noexcept
	{
		return y + height / 2;
	}
};
